...
#Looking at album popularity on it's own
...
```
## Specific Task 2 EDA
The following code covers some EDA into the genres using stringr and also number of clusters using the pam clustering function
```{r clustering, message=F, warning=F}
#Genre analysis using stringr to extract the most commonly appearing words
spotify.genres <- read_excel("inst/edited_spotify.xlsx") %>% 
...
genre_freq_plot <- ggplot(data = genre_words_table, aes(x=genre_words, y=Freq)) +
  geom_col() + 
  theme_bw()

print(genre_freq_plot)

#Taking a look at the clustering of the predictor variables
...
plotcluster(clustering_training_data, clusters$cluster, color = TRUE,
            shade = TRUE, lines = 0)
# Looking at all combinations of clusters from 1 to 10 for visualisation purposes.
# Takes about 4minutes to run and uses up about 5-6 pages in plots but
# It basically shows that < 3 clusters is not enough and > 6 clusters is too many
# Acts as a way to back up our future cluster choice of 5.
# for(i in 1:10){
#   clusters <- pam(clustering_training_data, i)
# plotcluster(clustering_training_data, clusters$cluster, color = TRUE,
#             shade = TRUE, lines = 0)
# }
```
