...
training_data <- training_data %>%
  mutate(album_weeks_n = if_else(AlbumBestChartPosition == 0, 1,
                         if_else(AlbumBestChartPosition == 1, 10,
                         if_else(AlbumBestChartPosition == 2, 9,
                         if_else(AlbumBestChartPosition == 3, 8,
                                 #Penality for not being top 3 by missing a number
                         if_else(AlbumBestChartPosition > as.numeric(abcp_normy[6]) &&
                                   AlbumBestChartPosition <= as.numeric(abcp_normy[7]), 6,
                         if_else(AlbumBestChartPosition > as.numeric(abcp_normy[7]) &&
                                   AlbumBestChartPosition <= as.numeric(abcp_normy[8]), 5,
                         if_else(AlbumBestChartPosition > as.numeric(abcp_normy[8]) &&
                                   AlbumBestChartPosition <= as.numeric(abcp_normy[9]), 4,
                         if_else(AlbumBestChartPosition > as.numeric(abcp_normy[9]) &&
                                   AlbumBestChartPosition <= as.numeric(abcp_normy[10]), 3,
                         if_else(AlbumBestChartPosition > as.numeric(abcp_normy[10]) &&
                                   AlbumBestChartPosition <= as.numeric(abcp_normy[11]), 2, 0 )
                         ))))))))) %>% #... repeat for other variables
  mutate(artist_follow_n = -1) %>%
  mutate(album_pop_n = -1) %>%
  mutate(artist_pop_n = -1) %>%
  mutate(album_weeks_on_chart_n = -1) %>%
  mutate(album_weeks_number_1n = -1)
  

#This is obviously wrong but not sure why... Looks like it should be working to me        
training_data %>% 
  ggplot(aes(x = album_weeks_n)) + 
  geom_bar()         

#Functionalising it doesn't seem to work

# updater <- function(new_column, old_column, normy){
#   for(i in 1:10){
#   for(j in 1:nrow(training_data)){
#       if(training_data[,old_column][j] >= as.numeric(normy[i]) &&
#          training_data[,old_column][j] < as.numeric(normy[i + 1])){
#     training_data[,new_column][j] = i
#   }
#   }
# }
# }         
# 
# updater("artist_follow_n", "ArtistNumFollowers", anf_normy)
#Create and update artist_num followers
...
lapply(c("artist_follow_n", "artist_pop_n","album_pop_n"),bar_plot)


as.numeric(anf_normy[1])
