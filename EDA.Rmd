...
```
```{r}
correlations <- data.frame(matrix(0, nrow = 0, ncol = 0))

for(i in 1:11){
  for(j in 1:11){
  correlations[i,j] <- round(cor(training_data_subsetted[i], training_data_subsetted[j]), 2)
  }
}
correlations
for(i in 1:11){
 names(correlations)[i] <- colnames(training_data_subsetted[i])
}
...
#Again as 0 is our first input this method is fine
for(i in 2:10){
  for(j in 1:nrow(training_data)){
...
#Will mutute in at the end to take care of end case
#Album number of followers
for(i in 1:10){
  for(j in 1:nrow(training_data)){
...
training_data_pop <- training_data %>%
  ...
  #Correcting for the max term which is not included in the if statements
...
pam_clust <- fviz_nbclust(model_data , FUNcluster = cluster::pam)
kmeans_clust <- fviz_nbclust(model_data , FUNcluster = kmeans)
clara_clust <- fviz_nbclust(model_data , FUNcluster = cluster::clara)
```
