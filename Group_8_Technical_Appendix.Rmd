...
There was room for interpretation on this question, our interpretation was as follows:
The training dataset is to be used as our data to create and test the models in the first instance. Then our Test data is mimicking the *new* artists who the recording label could invest in. The reason this was decided upon was the variables currently in our dataset are not as ideal in terms of predicting *song* popularity as we would like, so instead we'll focus on using it as a pilot study with popularity at an album level to test our ideas and theories. With the aim, not necessarily to optimise the function for the variables we currently but to pave a good grounding for the future implementation of the models on a larger, more commericial scale. In this way trying to 
