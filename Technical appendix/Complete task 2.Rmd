...
import <- read_csv("edited_spotify.csv")
...
# use k value of sqrt(2375) = ~49 for now
...
recommend_new_song <- function(song_name, exploration) {
  ...
  for(i in 1:(dim(tracks_in_cluster[,-1])[1])) {
    ...
    distances[i,2] <- rnorm(1, distances[i,2], exploration)
